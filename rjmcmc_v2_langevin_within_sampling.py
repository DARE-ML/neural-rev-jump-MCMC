    ...
    def sampler(self, dims1, dims2, w_limit, eta_limit):
        ...
        for i in range(samples -1):
            if is_in_lower_dimension: #jump up dimension: netw1 < netw2
                ...
                v = np.random.normal(0, tausq_v, w_size2-w_size1)
                #--- Propose w
                ...
                if (self.use_langevin_gradients is True) and (lx< self.langevin_prob):  
                    ...
                    w_proposal = np.concatenate((np.random.normal(w_gd, w_limit, w_size1), scale * np.exp(v)), axis = None)
                    ...
                    diff_prop = 0
                    w_proposal = np.concatenate((np.random.normal(w_net1, w_limit, w_size1), scale * np.exp(v)), axis = None)
                ...
                log_v = self.v_jump_up(tausq_v, v, w_size2-w_size1)
                #--- Calc log posterior 
                acceptance = (prior_proposal + likelihood_proposal) - (prior_current + likelihood_current + log_v) + (w_size2-w_size1) * np.log(scale) + np.sum(v) + diff_prop 
                try:
                ...
                if u < mh_prob:
                    ...
                    print('Sample from same model 1 with size', w_size1, 'Start within-model sampling')

                    if (self.use_langevin_gradients is True) and (lx< self.langevin_prob):  
                        w_gd_within = Net1.langevin_gradient(w_net1.copy(), self.sgd_depth)
                        w_proposal_within = np.random.normal(w_gd_within, w_limit, w_size1)  
                        w_prop_gd_within = Net1.langevin_gradient(w_proposal_within.copy(), self.sgd_depth) 

                        wc_delta_within = (w_net1 - w_prop_gd_within) 
                        ...
                        diff_prop_within = 0
                        w_proposal_within = np.random.normal(w_net1, w_limit, w_size1)

                    #--- Calc proposed prior and likelihood
                    [likelihood_proposal_within, pred_train_proposal_within, rmse_train_proposal_within] = self.likelihood_func(Net1, self.traindata, w_proposal_within, tau_pro, dims1)
                    [pred_test_proposal_within, rmse_test_proposal_within] = Net1.test_proposal(w_proposal_within)				
                    prior_proposal_within = self.prior_likelihood(sigma_squared, nu_1, nu_2, w_proposal_within, tau_pro, model_prior, dims1)
                    #--- Calc log posterior 
                    ...
                    if t < mh_prob_within:
                        ...
                        w_net1 = w_proposal_within
                        pos_w.append(w_net1)
                        print('Sampling from within model', w_size1, w_net1, 'accepted')
                ...
                #--- Calc log_v such that tausq is good
                log_v = self.v_jump_down(scale, tausq_v, v, w_size2-w_size1)
                #--- Calc log posterior 
                ...
                if u_down < mh_prob_down:
                    ...
                    print('Sample from same model 2 with size', w_size2, 'Start within-model sampling')

                    if (self.use_langevin_gradients is True) and (lx< self.langevin_prob):  
                        w_gd_within = Net2.langevin_gradient(w_net2.copy(), self.sgd_depth)
                        w_proposal_within = np.random.normal(w_gd_within, w_limit, w_size2)  
                        w_prop_gd_within = Net2.langevin_gradient(w_proposal_within.copy(), self.sgd_depth) 

                        wc_delta_within = (w_net2 - w_prop_gd_within) 
                        wp_delta_within = (w_proposal_within - w_gd_within )
                        ...
                        diff_prop_within = 0
                        w_proposal_within = np.random.normal(w_net2, w_limit, w_size2)

                    #--- Calc proposed prior and likelihood
                    [likelihood_proposal_within, pred_train_proposal_within, rmse_train_proposal_within] = self.likelihood_func(Net2, self.traindata, w_proposal_within, tau_pro, dims2)
                    [pred_test_proposal_within, rmse_test_proposal_within] = Net2.test_proposal(w_proposal_within)				
                    prior_proposal_within = self.prior_likelihood(sigma_squared, nu_1, nu_2, w_proposal_within, tau_pro, model_prior, dims2)
                    #--- Calc log posterior 
                    ...
                    if t < mh_prob_down_within:
                        ...
                        likelihood_current = likelihood_proposal_within
                        prior_current = prior_proposal_within
                        w_net2 = w_proposal_within
                        pos_w.append(w_net2)
                        print('Sampling from within model', w_size2, w_net2, 'accepted')
            ...
            print(np.shape(pos_w), '{:.5%}'.format(accept_ratio), 'is accepted with langevin counts', langevin_count, 'with within-model samples', within_model_samples)
        return (pos_w, fxtrain_samples, fxtest_samples, rmse_train, rmse_test, accept_ratio, within_model_ratio)

def main():
    for problem in range(2, 8): 
        ...
        [pos_w, fx_train, fx_test, rmse_train, rmse_test, accept_ratio, within_model_ratio] = mcmc.sampler(dims1, dims2, w_limit, eta_limit)
        ...
        print(problem, rmse_tr, rmsetr_std, rmse_tes, rmsetest_std)


        outres_db = open('rjmcmc_result_2_models.txt', "a+")

        np.savetxt(outres_db, (problem, use_langevin_gradients, len(mtaskNet),  rmse_tr, rmsetr_std, rmse_tes, rmsetest_std, accept_ratio, within_model_ratio), fmt='%1.5f')
        #----- Plot
        ...
        plt.title("Prediction  Uncertainty to jump between 2 models ")
        plt.savefig('rjmcmcrestest_2 models.png') 
        plt.clf()
        # -----------------------------------------
        ...
        plt.title("Prediction  Uncertainty to jump between 2 models")
        plt.savefig('rjmcmcrestrain_2 models.png') 
        plt.clf()
        ...
        plt.title("Boxplot of Posterior W (weights and biases) to jump between 2 models")
        plt.savefig('w_pos_rjmcmc_2 models.png')
