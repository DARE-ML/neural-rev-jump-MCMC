    ...
    def sampler(self, dims1, dims2, w_limit, eta_limit):
        ...
        for i in range(samples -1):
            if is_in_lower_dimension: #jump up dimension: netw1 < netw2
                ...
                #--- Calc log posterior 
                log_posterior = (prior_proposal + likelihood_proposal) - (prior_current + likelihood_current + log_v) + (w_size2-w_size1) * np.log(scale) + np.sum(v) + diff_prop 
                try:
                ...
                if u < mh_prob:
                    ...
                    #--- Calc log posterior 
                    log_posterior_within = (prior_proposal_within + likelihood_proposal_within) - (prior_current + likelihood_current) + diff_prop_within

                    try:
                        mh_prob_within = min(1, math.exp(log_posterior_within))
                ...
                log_posterior_down = (prior_current + likelihood_current + log_v) - (prior_proposal + likelihood_proposal) - np.log(np.sum(v)) - diff_prop 
                try:
                ...
                if u_down < mh_prob_down:
                    ...
                    #--- Calc log posterior 
                    log_posterior_down_within = (prior_proposal_within + likelihood_proposal_within) - (prior_current + likelihood_current) + diff_prop_within 

                    try:
                        mh_prob_down_within = min(1, math.exp(log_posterior_down_within))
