...
class MCMC:
    ...
    def sampler(self, num_models, w_limit, eta_limit):
        ...
        list_dims = np.arange(0, num_models, 1)
        ...
        for i in range(samples -1):
            if up: #jump up dimension: netw1 < netw2
                ...
                if u < mh_prob:
                    ...
                    w_Net[proposed_dims] = w_proposal #assign proposed w to w which now has higher dim
                    pos_w.append(w_proposal)
                    current_dims = proposed_dims

                    if current_dims < max(list_dims):
                        up = True
                ...
                if u_down < mh_prob_down:
                    ...
                    likelihood_current = likelihood_proposal
                    prior_current = prior_proposal
                    w_Net[proposed_dims] = w_proposal
                    pos_w.append(w_proposal)
                    current_dims = proposed_dims
                    print(i, 'jump from model with size', current_w_size,'to model with size', proposed_w_size)
                    if current_dims > min(list_dims):
                        up = False
            ...
            print(np.shape(pos_w), '{:.4%}'.format(accept_ratio), 'is accepted with langevin counts', langevin_count, 'with within-model samples', within_model_samples)
...
def main():
    for problem in range(1, 2): 
        ...
        [pos_w, fx_train, fx_test, rmse_train, rmse_test, accept_ratio, within_model_ratio] = mcmc.sampler(num_models, w_limit, eta_limit)
        ...
        outres_db = open('rjmcmc_result_general.txt', "a+")
        ...
        plt.savefig('rjmcmcrestest_general.png') 
        plt.clf()
        # -----------------------------------------
        ...
        plt.title("Prediction  Uncertainty ")
        plt.savefig('rjmcmcrestrain_general.png') 
        plt.clf()
        ...
        plt.savefig('w_pos_rjmcmc_general.png')
