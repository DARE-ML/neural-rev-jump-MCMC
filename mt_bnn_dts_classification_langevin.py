...
class Network:
    def __init__(self, Topo, Train, Test, MaxTime, MinPer, prob_type):
        ...
        self.lrate = 0.05  # will be updated later with BP call
    ...
    def langevin_gradient(self, data, w , mtopo, subtasks, depth):  # BP with SGD (Stocastic BP)
        ...
        for i in range(0, depth):
            for i in range(0, size):
                pat = i
                Input = data[pat, 0:self.Top[0]]
                #Desired = data[pat, data.shape[1]-self.Top[2]:]
                Desired = data[pat, -1]
                self.ForwardPass(Input)
...
outres1 = open('mt_result_poker_lngevin_inp.txt', "a+")

class BayesNN:  # Multi-Task leaning using Stocastic GD

    def __init__(self, use_langevin_gradients, langevin_prob, mtaskNet, traindata, testdata, samples, minPerf,   num_subtasks, prob_type):
    ...
    def taskdata(self, data, taskfeatures, output):
        ...
        return np.concatenate((featuregroup[:, list(range(0, taskfeatures))], data[:, list(range(datacols - output, datacols))]),
                              axis=1) #taskfeatures = number of inputs for each subtask, output = no of outputs for each subtask
    ...
    # ------------------- initialize MCMC
        ...
        for n in range(0, self.subtasks):
            module = self.mtaskNet[n] #params of model m
            if self.prob == 'regression':
                ...
                Netlist[n] = Network(self.mtaskNet[n], trdata, testdata, depthSearch, self.minCriteria, self.prob)
            elif self.prob == 'classification':
                trdata = self.taskdata(self.traindata, module[0], 1)  # make the partitions for task data
                testdata = self.taskdata(self.testdata, module[0], 1)
                Netlist[n] = Network(self.mtaskNet[n], trdata, testdata, depthSearch, self.minCriteria, self.prob)
...
#size of each subtask
        ...
        wc_delta =  np.random.randn( self.subtasks, netsize[self.subtasks-1])
        wp_delta =  np.random.randn( self.subtasks, netsize[self.subtasks-1])
        ...
        if self.prob == 'regression':
            pred_train, prob = Netlist[0].evaluate_proposal(trdata, w[0,:netsize[0]], self.mtaskNet, 0)  # we only take prior calculation for first ensemble, since we have one tau value for all the ensembles.
    ...
    #Ok, in this case, we only consider the number of hidden neurons increasing for each subtask. Input neurons and output neuron remain fixed in this version.
        ...
        for s in range(0, self.subtasks-1):
            ...
            [likelihood[s],  posfx_train[0, s,:],  posrmse_train[0, s]] = self.loglikelihood(Netlist[s], y_train, trdata, w[s,:netsize[s]], tau_pro, s, Netlist[s].Top)
            # use that of subtask m-1 for subtask m
            w[s + 1, :netsize[s]] = w[s, :netsize[s]]
            w_gd[s + 1, :netsize[s]] = w_gd[s, :netsize[s]]
        ...
        for i in range(1, samples-1):    # ---------------------------------------
            ...
            for s in range(0, self.subtasks):
                ...
                if (self.use_langevin_gradients is True) and (lx< self.langevin_prob):  
                    w_gd[s, :netsize[s]] = Netlist[s].langevin_gradient(trdata, w[s, :netsize[s]].copy(), self.mtaskNet, s, self.sgd_depth)  
                    w_pro[s, :netsize[s]] = w_gd[s, :netsize[s]] + np.random.normal(0, step_w, netsize[s])  
                    w_prop_gd[s, :netsize[s]] = Netlist[s].langevin_gradient(trdata, w_pro[s, :netsize[s]].copy(), self.mtaskNet, s, self.sgd_depth) 
                    ...
                    wc_delta[s, :netsize[s]] = (w[s, :netsize[s]] - w_prop_gd[s, :netsize[s]]) 
                    wp_delta[s, :netsize[s]] = (w_pro[s, :netsize[s]] - w_gd[s, :netsize[s]] )

                    sigma_sq = step_w

                    diff_prop[s] = -0.5 * np.sum(wc_delta[s, :netsize[s]]  *  wc_delta[s, :netsize[s]]  ) / sigma_sq - (-0.5 * np.sum(wp_delta[s, :netsize[s]] * wp_delta[s, :netsize[s]] ) / sigma_sq) # this is wc_delta.T  *  wc_delta /sigma_sq
            ...
            for s in range(0, self.subtasks-1):
                ...
                [likelihood_pro[s],  trainfx[s, :], rmsetrain[s]] = self.loglikelihood(Netlist[s], y_train, trdata, w_pro[s, :netsize[s]], tau_pro,s, Netlist[s].Top)
                [likelihood_ignore,  testfx[s, :], rmsetest[s]] = self.loglikelihood(Netlist[s], y_test, testdata, w_pro[s, :netsize[s]], tau_pro,s, Netlist[s].Top)
            ...
            for s in range(0, self.subtasks):
                ...
                try:
                    mh_prob[s] = min(1, math.exp(diff[s] + diff_prior[s] + diff_prop[s]))
            ...
            np.savetxt(outres1, (likelihood), fmt='%1.1f')
...
def main():
    ...
    for problem in range(18, 19):
        outres = open('mt_result_general.txt', "a+") #create file results.txt in new folder
        ...
        if problem == 1:
            ...
            input = 7  # max input
            output = 1
            moduledecomp = [3, 5, 7]  # number of inputs for each subtask
        ...
        # CLASSIFICATION
        separate_flag = False
        if problem == 8: #Wine Quality Red
            traindata = np.genfromtxt('/Users/megannguyen/Desktop/mt-bnn-dts-master/DATA/red-wine/ttrain.csv',delimiter=',')
            testdata = np.genfromtxt('/Users/megannguyen/Desktop/mt-bnn-dts-master/DATA/red-wine/ttest.csv',delimiter=',')
            ...
            hidden = 35
            output = 2
            moduledecomp = [5, 9, 11]  # number of inputs for each subtask
        if problem == 9: #IRIS
            ...
            hidden = 12
            output = 3
            moduledecomp = [2, 3, 4]  # number of inputs for each subtask
        if problem == 10: #Ionosphere
            traindata = np.genfromtxt('/Users/megannguyen/Desktop/mt-bnn-dts-master/DATA/Ions/Ions/ttrain.csv',delimiter=',')
            testdata = np.genfromtxt('/Users/megannguyen/Desktop/mt-bnn-dts-master/DATA/Ions/Ions/ttest.csv',delimiter=',')
            name = "Ionosphere"
            prob_type = 'classification'
            input = 34
            hidden = 50
            output = 2
            moduledecomp = [17, 25, 34]  # number of inputs for each subtask
        if problem == 11: #Cancer
            ...
            hidden = 4
            output = 2    
            moduledecomp = [5, 7, 9]  # number of inputs for each subtask
        if problem == 12: #Bank additional
            ...
            hidden = 80
            output = 2
            moduledecomp = [20, 40, 51]  # number of inputs for each subtask

        if problem == 13: #PenDigit
            traindata = np.genfromtxt('DATA/PenDigit/ftrain.csv',delimiter=',')
            testdata = np.genfromtxt('DATA/PenDigit/ftest.csv',delimiter=',')
            ...
            hidden = 30
            output = 10
            moduledecomp = [14, 15, 16]  # number of inputs for each subtask
        ...
        if problem == 14: #Chess
            ...
            hidden = 25
            output = 18
            moduledecomp = [2, 4, 6]  # number of inputs for each subtask
        ...
        if problem == 15: #Abalone
            ...
            hidden = 20
            output = 2
            moduledecomp = [8, 9, 10]  # number of inputs for each subtask
        ...
        if problem == 16: 
            ...
            hidden = 10
            output = 2
            moduledecomp = [9, 14, 18]  # number of inputs for each subtask
        ...
        if problem == 17: 
            ...
            hidden = 5
            output = 2
            moduledecomp = [5, 6, 7]  # number of inputs for each subtask
        if problem == 18: 
            ...
            hidden = 34
            output = 2
            moduledecomp = [6, 8, 10]  # number of inputs for each subtask
        ...
        if problem == 20: 
            ...
            hidden = 23
            output = 2
            moduledecomp = [5, 8, 9]  # number of inputs for each subtask
        
        if problem == 21: 
            traindata = np.genfromtxt('/Users/megannguyen/Desktop/mt-bnn-dts-master/DATA/Page Blocks/ftrain.csv',delimiter=',')
            testdata = np.genfromtxt('/Users/megannguyen/Desktop/mt-bnn-dts-master/DATA/Page Blocks/ftest.csv',delimiter=',')
            name = "Page Blocks"
            prob_type = 'classification'
            input = 10
            hidden = 6
            output = 2
            moduledecomp = [5, 7, 10]  # number of inputs for each subtask
        ...
        for i in range(1, subtasks):
            mtaskNet[i - 1][0] = moduledecomp[i - 1] #for previous subtask, the input layer = modulecomp
        ...
        bayesnn = BayesNN(use_langevin_gradients, langevin_prob, mtaskNet, traindata, testdata, num_samples, min_perf,   subtasks, prob_type)
        ...
        for s in range(0, subtasks):
            ...
            np.savetxt(outres1, (posrmse_train[int(burnin):,s]), fmt='%1.1f')
            np.savetxt(outres1, (posrmse_test[int(burnin):,s]), fmt='%1.1f')
