...
class Network:
    ...
    def encode(self):
        ...
        return w
    def langevin_gradient(self, w, depth, mtopo, subtasks):  # BP with SGD (Stocastic BP)
...
class BayesNN:  # Multi-Task leaning using Stocastic GD
    ...
    def __init__(self, use_langevin_gradients, langevin_prob, mtaskNet, traindata, testdata, samples, minPerf,   num_subtasks):
...
#size of each subtask
        ...
        diff_prop = np.zeros(self.subtasks) #initialize for langevin gradients
        ...
        langevin_count = np.zeros(self.subtasks)
        naccept = 0
    ...
    ## PROPOSE FOR PARAMS of MODEL M theta_m
        ...
        for i in range(1, samples-1):    # ---------------------------------------
            ...
            for s in range(0, self.subtasks):
                ...
                if (self.use_langevin_gradients is True) and (lx< self.langevin_prob):  
                    w_gd[s, :netsize[s]] = Netlist[s].langevin_gradient(w[s, :netsize[s]].copy(), self.sgd_depth, self.mtaskNet, s)  
                    w_pro[s, :netsize[s]] = w[s, :netsize[s]] + np.random.normal(0, step_w, netsize[s])

                    w_prop_gd[s, :netsize[s]] = Netlist[s].langevin_gradient(w_pro[s, :netsize[s]].copy(), self.sgd_depth, self.mtaskNet, s) 
                    wc_delta[s, :netsize[s]] = (w[s, :netsize[s]] - w_gd[s, :netsize[s]]) 
                    wp_delta[s, :netsize[s]] = (w_pro[s, :netsize[s]] - w_prop_gd[s, :netsize[s]])

                    first = -0.5 * np.sum(wc_delta[s, :netsize[s]] * wc_delta[s, :netsize[s]]) / (step_w * step_w)  # this is wc_delta.T  *  wc_delta /sigma_sq
                    second = -0.5 * np.sum(wp_delta[s, :netsize[s]] * wp_delta[s, :netsize[s]]) / (step_w * step_w)

                    diff_prop[s] =  first - second  
                    langevin_count[s] = langevin_count[s] + 1
                else:
                    diff_prop[s] = 0
                    w_pro[s, :netsize[s]] = w[s, :netsize[s]] + np.random.normal(0, step_w, netsize[s])
            ...
            for s in range(0, self.subtasks):
                ...
                mh_prob[s] = min(1, np.exp(diff[s] + diff_prior[s] + diff_prop[s]))
                ...
                if u < mh_prob[s]:
                    naccept += 1
                    print(i, ' is accepted sample with langevin counts', langevin_count[s])
        ...
        print(naccept, ' num accepted with langevin counts', langevin_count[s])
        accept_ratio = naccept / (samples * self.subtasks *1.0) * 100


        return (w_pos, posfx_train, posfx_test, posrmse_train, posrmse_test, pos_tau, x_train, x_test,  y_test, y_train, accept_ratio, langevin_count)
...
def main():
    ...
    for problem in range(1, 8):
        ...
        bayesnn = BayesNN(use_langevin_gradients, langevin_prob, mtaskNet, traindata, testdata, num_samples, min_perf,   subtasks)

        [w_pos, posfx_train, posfx_test, posrmse_train, posrmse_test, pos_tau, x_train, x_test, y_test, y_train, accept_ratio, langevin_count] = bayesnn.mcmc_sampler()
        ...
        np.savetxt(outres, (problem, langevin_count, accept_ratio), fmt='%1.1f')
