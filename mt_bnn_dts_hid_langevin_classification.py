...
outres1 = open('mt_result_pageblocks_lngevin_hid.txt', "a+")
        ...
        for i in range(1, samples-1):    # ---------------------------------------
            ...
            for s in range(0, self.subtasks):
                ...
                if (self.use_langevin_gradients is True) and (lx< self.langevin_prob):  
                    w_gd[s, :netsize[s]] = Netlist[s].langevin_gradient(self.traindata, w[s, :netsize[s]].copy(), self.mtaskNet, s, self.sgd_depth)  
                    w_pro[s, :netsize[s]] = w_gd[s, :netsize[s]] + np.random.normal(0, step_w, netsize[s])  
                    w_prop_gd[s, :netsize[s]] = Netlist[s].langevin_gradient(self.traindata, w_pro[s, :netsize[s]].copy(), self.mtaskNet, s, self.sgd_depth) 
...
def main():
    ...
    for problem in range(21, 22):
        ...
        for i in np.arange(0, subtasks, 1):
            mtaskNet.append([input, hidden + i * hidden, output])
