...
class MCMC:
    ...
    def sampler(self, dims1, dims2, w_limit, tau_limit):
        #----- Model 1
        ...
        for i in range(samples -1):
            if is_in_lower_dimension: #jump up dimension: netw1 < netw2
                ...
                eta_pro = eta + np.random.normal(0, tau_limit, 1)
                ...
                v = np.random.normal(0, w_limit, w_size2-w_size1)
                #--- Propose w
                ...
                log_posterior_down = (prior_current + likelihood_current + log_v) - (prior_proposal + likelihood_proposal) 
                try:
...
def main():
    for problem in range(2, 3): 
        ...
        [pos_w, fx_train, fx_test, rmse_train, rmse_test, accept_ratio] = mcmc.sampler(dims1, dims2, w_limit, tau_limit)
